...
public class SnapshotPreview {
	...
	protected void snapshotPreview(){
		...
		if (MainVideoManipulator.getInstance().timeSnapshot.equals("")) {
			...
			// in the right format
			boolean passedTimeCheck = MainVideoManipulator.getInstance().vc.timeChecks(MainVideoManipulator.getInstance().timeSnapshot.getText()
					.trim());
			...
			if (passedOrNot) {
				int convertedFrameTime = MainVideoManipulator.getInstance().vc.convertToSeconds(MainVideoManipulator.getInstance().timeSnapshot
						.getText().trim());
	...
	class SnapPrevBackgroundTask  extends SwingWorker<Void, String> {
		...
		protected Void doInBackground() throws Exception {
			...
			String inputFrameTime = MainVideoManipulator.getInstance().timeSnapshot.getText().trim();

			// command to create the preview screenshot
			cmd.append("avconv -i " + MainVideoManipulator.getInstance().videoPath + " -ss " + inputFrameTime
					+ " -f image2 -vframes 1 " + MainVideoManipulator.getInstance().hiddenDir
					+ "/previewSnap.png");
			cmd.append(";");
			...
			try {
				...
				builder = new ProcessBuilder("/bin/bash", "-c", MainVideoManipulator.getInstance().previewSnapCmd);
